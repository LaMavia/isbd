module IntSig : sig
  ...
  let serialize a offset =
    ...
    Result.Ok (Bytes.get_int64_ne bytes 0 |> Int64.to_int, offset + size)

  let deserialize a v i =
    let bytes = Bytes.create size in
    Int64.of_int v |> Bytes.set_int64_ne bytes 0;
...
module CharSig : sig
  ...
  let deserialize a str i =
    let len = String.length str in
    let size = IntSig.size + len in
    let bytes = Bytes.create size in
    Int64.of_int len |> Bytes.set_int64_ne bytes 0;
...
module PersonRecordSig : sig
  ...
  let serialize a offset =
    ...
    Result.Ok ({ age; name }, offset'')

  let deserialize a p offset =
    IntSig.deserialize a p.age offset |> CharSig.deserialize a p.name
end
