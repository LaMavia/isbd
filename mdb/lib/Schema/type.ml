module SignedVLQSig : sig
  include Sig.SchemaSig with type t := int64 and type extra := unit
end = struct
  open SigArray.OffsetSyntax
  (*
  ...
  *)
  ...
  let compress b _ = b
  let decompress b _ = b
end

module UnsignedVLQSig : sig
  include Sig.SchemaSig with type t := int64 and type extra := unit
end = struct
  open SigArray.OffsetSyntax
  (*
  ...
  *)
  ...
  let rec serialize a offset () = serialize_aux 0L 0 a offset

  and serialize_aux u shift a offset =
    let octet = Bytes.get_uint8 (SigArray.read_bytes a offset 1) 0 in
    let continue = octet >= 0b10000000 and rest = Int.logand octet 0b01111111 in
    let u = Int64.shift_left (Int64.of_int rest) shift |> Int64.add u in
    Printf.eprintf "octet=%du | shift=%d | continue=%B\n" octet shift continue;
    if continue then serialize_aux u (shift + 7) a (offset +> 1)
    else Result.ok (u, offset +> 1)

  let rec deserialize a () v offset = deserialize_aux a v offset

  and deserialize_aux a v offset =
    let continue_mask = if v > 127L then 0b10000000 else 0b0 (*2^7 - 1*)
    and octet_val = Int64.logand v 0b01111111L |> Int64.to_int in
    let bts = Bytes.make 1 '\000' in
    Bytes.set_uint8 bts 0 Int.(logor continue_mask octet_val);
    SigArray.write_bytes a offset 1 bts;
    if continue_mask > 0 then
      deserialize_aux a (Int64.shift_right_logical v 7) (offset +> 1)
    else offset +> 1
...
module StringSig : sig
  include Sig.SchemaSig with type t := string and type extra := int
end = struct
  open SigArray.OffsetSyntax

  let serialize a offset len =
    Result.ok
      (SigArray.read_bytes a offset len |> String.of_bytes, offset +> len)

  let deserialize a _ str offset =
    let bts = String.to_bytes str in
    let bts_len = Bytes.length bts in
    SigArray.write_bytes a offset bts_len bts;
    offset +> bts_len

  let compress b len = Bytes.sub b 0 len |> LZ4.Bytes.compress
  let decompress b len = LZ4.Bytes.decompress ~length:len b
end
