...
module YoJsonMap (V : Serializable) = struct
  ...
  let of_yojson (m : Yojson.Safe.t) =
    ...
    | _ ->
      Result.Error
        (Printf.sprintf "Expected object but received '%s'" (Yojson.Safe.to_string m))
  ;;
  ...
  let rec of_yojson (m : Yojson.Safe.t) : (t, string) result =
    ...
    try
      ...
      | Some "object" ->
        SchemaObject
          { type_ = "object"
          ; required = [ m ] |> filter_member "required" |> flatten |> filter_string
          ; properties = SchemaMap.empty
          }
        |> Result.ok
    with
    | Type_error (_, reason) -> Result.error reason
  ;;
...
module Operation = struct
  ...
  let response_key_of_yojson (m : Yojson.Safe.t) : (response_key, string) Result.t =
    match m with
    | `String "default" -> Result.Ok Default
    | `Int code -> Result.Ok (Code code)
    | v ->
      Result.Error (Printf.sprintf "Invalid response key: %s" (Yojson.Safe.to_string v))
  ;;
...
let register_schema (name : string) (schema : Schema.t) (o : t) =
  { o with
    components =
      { o.components with schemas = SchemaMap.add name schema o.components.schemas }
  }
;;
